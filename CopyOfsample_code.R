...
# Assuming you already have an igraph object called `food_web_plot`

# Function to calculate secondary extinctions
calculate_secondary_extinctions <- function(network, species_to_remove) {
  # Remove the target species
  new_network <- delete_vertices(network, species_to_remove)
  
  # Identify which species are disconnected and thus 'extinct'
  extinct_species <- V(new_network)$name[degree(new_network, mode = "all") == 0]
  
  # Return the number of secondary extinctions
  tibble(
    removed_species = species_to_remove,
    secondary_extinctions = length(extinct_species),
    remaining_species = vcount(new_network)
  )
}

# Step 1: Simulate random removal of species and calculate secondary extinctions
random_removals <- map_df(1:100, ~ {
  species_to_remove <- sample(V(food_web_plot)$name, 1)
  calculate_secondary_extinctions(food_web_plot, species_to_remove)
})

# Step 2: Simulate targeted removal based on species with highest degree centrality
targeted_removals <- tibble(
  species = V(food_web_plot)$name,
  degree = degree(food_web_plot, mode = "all")
) %>%
  arrange(desc(degree)) %>%
  slice(1:10) %>%
  pull(species) %>%
  map_df(~ calculate_secondary_extinctions(food_web_plot, .))

# Combine random and targeted removal results
removal_results <- bind_rows(
  random_removals %>% mutate(removal_type = "Random"),
  targeted_removals %>% mutate(removal_type = "Targeted")
)

# Step 3: Statistical analysis - ANOVA to compare extinction rates between random and targeted removal
anova_results <- removal_results %>%
  aov(secondary_extinctions ~ removal_type, data = .) %>%
  summary()

# Print the ANOVA results
print(anova_results)

# Step 4: (Optional) Regression analysis to identify predictors of secondary extinctions (e.g., degree, betweenness)

# Combine degree with secondary extinction data for targeted removals
targeted_removals_with_degree <- targeted_removals %>%
  mutate(degree = degree(food_web_plot, v = targeted_removals$removed_species, mode = "all"))

# Now you can create the linear model using both degree and secondary extinctions
lm_model <- lm(secondary_extinctions ~ degree, data = targeted_removals_with_degree)

# Print the regression summary
summary(lm_model)


# Step 5: Create plots to visualize the effect of species removal on secondary extinctions

# Plot for random removal
random_plot <- ggplot(random_removals, aes(x = removed_species, y = secondary_extinctions)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(title = "Random Species Removal vs. Secondary Extinctions", x = "Species Removed", y = "Secondary Extinctions") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

random_plot

# Plot for targeted removal
targeted_plot <- ggplot(targeted_removals, aes(x = removed_species, y = secondary_extinctions)) +
  geom_bar(stat = "identity", fill = "red") +
  labs(title = "Targeted Species Removal vs. Secondary Extinctions", x = "Species Removed", y = "Secondary Extinctions") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

targeted_plot

# Combine the two plots
gridExtra::grid.arrange(random_plot, targeted_plot, ncol = 2)
